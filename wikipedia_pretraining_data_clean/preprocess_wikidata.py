...
import sys
sys.path.append('GENRE_PATH')

import jsonlines
# from genre.base_model import mGENRE
...
if __name__ == "__main__":
    ...
    if args.step == "compress":
        wikidata = 0
        with open(
            os.path.join(args.base_wikidata, "full_json_retrieved_entities.jsonl"), "r"
        ) as fi, jsonlines.open(
            os.path.join(args.base_wikidata, "full_json_retrieved_entities_compressed.jsonl"), "w"
        ) as fo:
    ...
    #             fo.write(item)
        ...
        filename = os.path.join(
            args.base_wikidata,
            "full_json_retrieved_entities_compressed{}.jsonl".format(
        ...
        for lang in ('de', 'la'):
            with open(
                "../wikidata_kb_margherita/{}wiki-redirects.txt".format(lang)
            ) as f:
                for row in tqdm(csv.reader(f, delimiter="\t"), desc=lang):
                    title = unquote(row[1]).split("#")[0].replace("_", " ")
                    if title:
                        title = title[0].upper() + title[1:]
                        if (lang, row[0]) not in lang_redirect2title:
