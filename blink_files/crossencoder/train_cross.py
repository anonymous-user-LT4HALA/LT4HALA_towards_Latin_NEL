...
def evaluate(reranker, eval_dataloader, device, logger, context_length, zeshel=False, silent=True):
    ...
    for step, batch in enumerate(iter_):
        ...
        if len(batch) == 3:
            ...
            pageweights = batch[2]
            with torch.no_grad():
                eval_loss, logits = reranker(context_input, label_input, context_length, pageweights)
        else:
            context_input = batch[0]
            label_input = batch[1]
            with torch.no_grad():
...
def main(params):
    ...
    if 'pageweights' in train_data.keys():
        pageweights = train_data["pageweights"]
    else:
        pageweights = None
    if params["debug"]:
        max_n = 2
        logger.info(context_input[0])
        logger.info(reranker.tokenizer.convert_ids_to_tokens(context_input[0]))
        context_input = context_input[:max_n]
        candidate_input = candidate_input[:max_n]
        logger.info(candidate_input[0])
        logger.info(reranker.tokenizer.convert_ids_to_tokens(candidate_input[0][label_input[0]]))
        label_input = label_input[:max_n]
        pageweights = pageweights[:max_n] if pageweights != None else None
    ...
    if params["zeshel"]:
        ...
        train_tensor_data = TensorDataset(context_input, label_input, pageweights) if pageweights != None else TensorDataset(context_input, label_input)
    ...
    if 'pageweights' in valid_data.keys():
        pageweights = valid_data["pageweights"]
    else:
        pageweights = None
    if params["debug"]:
        max_n = 2
    ...
    # logging.info(f"first before modify: {context_input[0]}, {candidate_input[0][label_input[0]]}")
    ...
    if params["zeshel"]:
        ...
        valid_tensor_data = TensorDataset(context_input, label_input, pageweights) if pageweights != None else TensorDataset(context_input, label_input)
    valid_sampler = SequentialSampler(valid_tensor_data)
    if pageweights != None:
        logging.info(f"first trainy thing passed to evaluate function: {context_input[0][label_input[0]]}, {label_input[0]}, {pageweights[0]}")
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            if len(batch) == 3:
                ...
                label_input = batch[1]
                pageweights = batch[2]
                loss, _ = reranker(context_input, label_input, context_length, pageweights)
            else:                
                context_input = batch[0]
                label_input = batch[1]
